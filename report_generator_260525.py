...
def parse_data_bytes(line):
    match = re.search(r'\b[dD]\s+\d+\s+((?:[0-9A-Fa-f]{2}(?:\s+|$))+)', line)
    if match:
        return [b.upper() for b in match.group(1).strip().split()]
    return []
...
def parse_line(line):
    ...
    if direction_index is None or direction_index == 0:
        return None
    can_id = parts[direction_index - 1].upper()
...
def parse_asc_file(asc_file_path, allowed_tx_ids, allowed_rx_ids):
    ...
    allowed_tx_ids = set(f"{id:X}".upper() for id in allowed_tx_ids)
    allowed_rx_ids = set(f"{id:X}".upper() for id in allowed_rx_ids)
    ...
    for line in lines:
        ...
        if not msg or msg["can_id"].upper() not in allowed_ids:
            continue
